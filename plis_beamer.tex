...
\begin{align*}
\lambda_D &= \pi h^\frac{1}{2} \alpha^\frac{1}{4} \left(\frac{4\Upsilon}{3}\right)^\frac{1}{4} \simeq \SI{1.24}{\milli\metre}\\
\lambda_P &= \pi h^\frac{1}{2} H^\frac{1}{2} \left(\frac{2\Upsilon}{9}\right)^\frac{1}{6} \simeq \SI{1.85}{\milli\metre}
\end{align*}
\end{block}
\column{0.5\textwidth}
\[\Upsilon = \frac{E(e-h)}{\eta v}\simeq \num{3e8}\]
...
\item[$\Rightarrow$] $H^* = \alpha^\frac{1}{2} \left(24\Upsilon\right)^\frac{1}{6} \simeq \SI{12}{\micro\metre}$
\end{itemize}
...
\input{fig_presentation/Darcy_vs_Poiseuille.pgf}

\begin{itemize}
\item Each model fits half of the data
\item Poiseuille underestimates small wavelengths ($H < H^*$)
\item Final model takes both Darcy and Poiseuille into account
\end{itemize}
