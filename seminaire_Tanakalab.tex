...
\begin{loglogaxis}[
	...
	\addplot[Accent2, ultra thick, domain={1e-4:0.1}] {0.2*x^(-0.85)} node[pos=0.75, below left] {$\frac{\dot{\gamma}(t)}{\dot{\gamma}_\text{min}}\sim \left(\frac{t}{\tau_f}\right)^{-0.85}$};
\end{loglogaxis}
\end{tikzpicture}
...
\begin{loglogaxis}[
	...
	\addplot[Accent2, ultra thick, domain={1e-4:0.1}] {0.05/x} node[pos=0.6, below right, inner sep=0] {$\frac{\dot{\gamma}(t)}{\dot{\gamma}_\text{min}}\sim \left(\frac{\tau_f}{\tau_f-t}\right)$};
\end{loglogaxis}
\end{tikzpicture}
...
\begin{axis}[
	...
	\addplot[Accent2, ultra thick, no marks, domain={1e-3:1}] {8.9e-3 - 0.06 * ln(x)};
\end{axis}
\end{tikzpicture}
\end{block}
