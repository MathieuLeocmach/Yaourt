...
\begin{figure}
	\begin{tikzpicture}
	\node[inner sep=0, anchor=north] (x800) {\includegraphics[height=0.5\columnwidth]{pattern_800um_x1.jpg}};
	
	\matrix[matrix of nodes, inner sep=0, matrix anchor=south, column sep=0, row sep=0.5em] at ($(x800.north)+(0,0.5em)$) (m) {		
		\includegraphics[height=0.25\columnwidth, angle=90, trim=512px 0 0 0, clip=true]{pattern_50um.jpg} & 
		\includegraphics[height=0.25\columnwidth, angle=90, trim=512px 0 0 0, clip=true]{pattern_100um.jpg} &
		\includegraphics[height=0.25\columnwidth,angle=90]{pattern_250um.jpg} & 
		\includegraphics[height=0.25\columnwidth,angle=90]{pattern_450um.jpg} \\
	};
	
	\draw[ultra thick, Accent2, <->] (x800.south west) ++(0.376\columnwidth, 0.09\columnwidth) -- ++(0.278\columnwidth,0) node[midway,above]{$\lambda$};
	\draw[ultra thick](m-1-1.north west) ++(0,0.5em) -- ++(0.167\columnwidth,0) node[midway, above] {\SI{2}{\milli\metre}};
	%marks
	\begin{scope}[anchor=north west,Accent1,font=\Large]
		\node[font=\huge] at (m-1-1.north west) {$\bullet$};
		\node at (m-1-2.north west) {$\blacktriangle$};
		\node at (m-1-3.north west) {$\blacksquare$};
		\node at (m-1-4.north west) {$\blacklozenge$};
		\node at (x800.north west) {$\bigstar$}; 
	\end{scope}
	...
	\begin{axis}[%
		...
		\addplot+[only marks,Accent1, mark options={fill=Accent1}, mark=*] coordinates {(0.05,0.261)};
		 \addplot+[only marks,Accent1, mark options={fill=Accent1}, mark=triangle*] coordinates {(0.1,0.458)}; 
		 \addplot+[only marks,Accent1, mark options={fill=Accent1}, mark=square*] coordinates {(0.250,0.916)};
		 \addplot+[only marks,Accent1, mark options={fill=Accent1}, mark=diamond*] coordinates {(0.450,2.075)};
		 \addplot+[only marks,Accent1, mark options={fill=Accent1}, mark=text, text mark={$\bigstar$}] coordinates {(0.800,3.333)};
