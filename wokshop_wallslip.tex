...
\title[Yoghurt wrinkling]{Wrinkling of a confined porous layer}
\author[M. Leocmach]{Mathieu Leocmach}
\date{24\textsuperscript{th} September 2014}
\titlegraphic{
...
\begin{itemize}
\item What is the role of boundary conditions?
...
\begin{tikzpicture}
	...
	\addplot+[no marks,Accent2] table[x expr={\thisrowno{0}/3600}]{Y189_cas4_gdl4.pH};
	\draw[help lines] (axis cs:0,4.6) -- (axis cs:0.26,4.6) node[pos=1, above right, font=\scriptsize] {isoelectric point $pH\approx 4.6$}  -- (axis cs:0.26,0);
	...
	\addplot+[no marks,Accent2] table[x expr={\thisrowno{0}/3600}]{cas4_GDL4_Y271.prise};
	\addplot+[no marks,Main] table[x expr={\thisrowno{0}/3600}, y index=2]{cas4_GDL4_Y271.prise};
	\begin{scope}[font=\scriptsize]
		%\node[above right] at (rel axis cs:0,0) {casein ``micelles''};
		%\draw[<-] (axis cs:3.88,654) -- +(1em,0) node[right] {gelation};
		\node[anchor=base,rotate=80] at (axis cs:0.2,300) {gelation};
	\end{scope}
	\end{groupplot}
