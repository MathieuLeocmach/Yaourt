...
\begin{textblock}{4}(5,16)
	...
	\begin{loglogaxis}[%
		...
		\addplot+[only marks, Accent1, mark options={fill=Accent1}] coordinates {(40, 67225) (50,29860) (100,398) (200,4)};
		\addplot+[only marks, Accent2, mark options={fill=Accent2}] table[x index=0, y index=1]{cas4_GDL1_t_cassure_sigma.txt};
		\legend{gdl 4\%, gdl 1\%};
		\addplot+[Accent1, no marks, forget plot, domain=40:200] gnuplot {6.32e+14*x^{-6.14}};
...
\begin{textblock}{5}(10,22.5)
	\Head{Non equilibrium Creep}
	\Subhead{Fractures need complete overshoot}
	\begin{itemize}
		\item Wait only \SI{40}{\minute}. $G^\prime$ at maximum.
		\item Start creep with $\sigma > \SI{200}{\pascal}$.
		\item[$\Rightarrow$] Break before $G^\prime$ falls.
		\item[$\Rightarrow$] No fracture
	\end{itemize}
\end{textblock}
...
\begin{textblock}{2.875}(12,2.625)%real width of 3
	...
	\end{itemize}
	\Subhead{A good model for solids}
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item Fractures need complete overshoot
	\end{itemize}
