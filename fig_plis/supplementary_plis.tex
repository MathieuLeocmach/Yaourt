...
\externalcitedocument[M-]{article_plis}
...
\section*{Microstructure measurement}
...
\section*{Permeability measurements}
\begin{figure*}%
...
\section*{Confocal images processing}
...
\section*{Characteristic time measurement}
\begin{figure}
...
\section*{Mixed Darcy--Poiseuille model}

\subsection*{General framework for wrinkling}
...
\label{eq:mecheq}
\end{equation}
where $w(x,t)$ is the deflection of the plate along $z$~\cite{M-Biot1957}. Since we deal with slow rates of deformation, inertia is neglected.
...
Following \cite{M-Biot1957}, we consider the case where the compression load $\sigma_\parallel$ is increased quasistatically. The observed mode is the one that arises at the lowest possible $\sigma_\parallel$, i.e. the mode minimising $\sigma_\parallel$.

\subsection*{Elastic film in an infinite elastic medium}

In this framework, we recall here the case of an elastic film lying over an infinite elastic medium~\cite{M-Gough1940,M-Bijlaard1946,M-Biot1957,M-Cerda2003}. The deformation of the substrate of modulus $B_s$ gives the transverse load as $\sigma_\perp = -q B_s w$, and thus the compression load writes
\begin{equation}
\sigma_\parallel = B \frac{h^2}{12} q^2 + \frac{B_s}{hq}.
\label{eq:sigma0}
\end{equation}

Minimising $\sigma_\parallel$ as a function of $q$ yields $hq = \left(6B_s/B\right)^{1/3}$ \cite{M-Biot1957,M-Cerda2003} so that the dominant wavelength is 
\begin{equation}
\lambda = 2\pi h \left(\frac{B}{6B_s}\right)^{1/3}.
\label{eq:lambdaElEl}
\end{equation}

\subsection*{Elastic film on a viscous layer}
...
\subsection*{Porous elastic film with no possible Poiseuille flow}
...
\subsection*{Porous elastic film between two viscous layers}
...
\section*{Samples characteristics}
...
\end{tabular}
\caption{Characteristics of the samples used for Main Figure~\ref{M-fig:DarcyPoiseuille}. Lines where preparation and properties are left blank correspond to the average of the secondary blisters of the previous line. `cas', `GDL' and `gly' indicate the weight fraction of sodium caseinate, GDL and glycerol respectively. $\xi$ is the pore size at the end of each experiment. $h$, $H_1$ and $H_2$ are measured just before buckling.}
\label{tab:data}
\end{table*}
