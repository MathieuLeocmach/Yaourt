...
\begin{figure}
	...
	\caption{Dynamics of pattern formation in a confined film of casein gel (caseinate 4\%w, GDL 4\%w in water). (a) XXX (b) Light transmission microscopy. Successive generations of patterns are highlighted in colour to stress the absence of coarsening after formation. (c) 3D reconstruction from fluorescent confocal microscopy. Scale bars are \SI{1}{\milli\metre}. XXX lambda, L, e XXX}%
	\label{fig:dynamics}%
\end{figure}
...
The wrinkling patterns shown in Figure~\ref{fig:dynamics} are obtained in home made optical cells. A cell is composed of a slit and a cover slip  coated with polymer brushes and spaced by parafilm. The cell thickness $e$ is typically \SI{100}{\micro\metre} whereas other dimensions are $L$, typically \SI{1}{\centi\metre}. The cell is filled with the casein+GDL dispersion while it is fluid, and sealed with optical UV-glue, see Methods and Figure~\ref{fig:dilation}a. In such configuration, the adhesion to both top and bottom walls is turned off and therefore as the gel forms, it is only anchored to the remaining four sides of the cell (parafilm or cured glue).

As the gel forms it expels the solvent and detaches from top and bottom wall, forming a thin film sandwiched between two solvent layers. Upon swelling back, the gel spontaneously wrinkles with a wavelength $\lambda\approx\SI{1}{\milli\metre}\ll L$, see first row of Figure~\ref{fig:dynamics}. As 


Let us examine possible scenarios upon shrinking and swelling of the gel. As the gel forms it expels the solvent and detaches from top and bottom wall, see Figure~\ref{fig:dilation}b and c. Then we consider three options. (i) In Figure~\ref{fig:dilation}d, the swelling back is reversible, only releasing tensile stresses accumulated during shrinking, and the layer remains flat. Since this case is not observed it means that in this physical gel, syn\ae{}resis and swelling processes foster irreversible stress dissipation and structural changes. In the following, we will therefore consider that the shrunk state is stress-free. 
(ii) In Figure~\ref{fig:dilation}e, we assume that nothing hinders the buckling of the gel film. This simple buckling situation leads to a primary composed of a single bump of size $L$. As the bump amplitude grows larger saturates on the top and bottom wall, buckling of smaller wavelength appear, as observed by~\cite{Roman1999}. That is not what we observe since our primary wavelength $\lambda$ is much smaller that $L$. 
(iii) We are left with the case of Figure~\ref{fig:dilation}f where a wrinkling pattern emerges spontaneously with a wavelength $\lambda\ll L$ selected before saturation on the top and bottom wall. To obtain such an instability, one must balance the bending rigidity of the gel film with a transverse load that hinders buckling on large wavelengths, as described for example by~\cite{Biot1957,Vella2009,Kolinski2009}.
