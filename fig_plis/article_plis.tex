...
\begin{figure*}
	...
	\caption{Comparing model predictions with measured wavelengths. Symbols are empty when $H<H^*$ (Darcy mode) and full when $H>H^*$ (Poiseuille mode). Orange dots come from primary pattern, yellow squares from secondary blisters. Dashed lines are the best linear fit through the origin taking only into account the points that should be (a) in Darcy mode, (b) in Poiseuille mode (c) all points. Prefactors are 0.63, 0.69 and 0.67 respectively. Continuous line is the best affine fit ($\lambda_{xp}=0.52\lambda_{th}+\SI{0.33}{\milli\metre}$) to all the points in (b).}
