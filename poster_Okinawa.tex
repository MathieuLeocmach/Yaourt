...
Usual view (\textit{Biot 1964}): pore pressure\linebreak increases transiently the bending energy.
\begin{itemize}
\item Solvent must flow through the gel
\begin{itemize}
	\item Volumic flux $v\simeq \SI{0.2}{\micro\metre\per\second}$
	\item Pore size $\ell_p \approx \SI{4}{\micro\metre}$
\end{itemize}
...
\smallskip
\includegraphics[width=\textwidth]{large1}
\end{textblock}

\begin{textblock}{3}(12,8)
	\Head{Cell thickness}
\begin{tikzpicture}
	\matrix[matrix of nodes, inner sep=0, column sep=0.02\columnwidth, row sep=0.02\columnwidth] (m) {
		\includegraphics[width=0.49\columnwidth]{pattern_50um.jpg}&
		\includegraphics[width=0.49\columnwidth]{pattern_100um.jpg}\\
		\includegraphics[width=0.49\columnwidth]{pattern_250um.jpg}&
		\includegraphics[width=0.49\columnwidth]{pattern_450um.jpg}\\
	};
	\node[inner sep=0, below=0.02\columnwidth of m.south] (x800) {\includegraphics[width=0.98\columnwidth]{pattern_800um_x1.jpg}};
	\draw[ultra thick, Accent2, <->] (x800.south west) ++(0.25\columnwidth, 0.02\columnwidth) -- ++(45:0.29\columnwidth) node[midway,above]{$\lambda$};
	\draw[line width=0.2em,Main](x800.north east) -- ++(-0.434\columnwidth,0) node[midway,below] {\SI{5}{\milli\metre}};
	
	\begin{axis}[%
		name=g,
		anchor=left of north west,
		at={($(x800.south west)+(-0.01\columnwidth,-0.01\columnwidth)$)},
		scale only axis,
		width=\textwidth-3.25em,
		height=0.3\textwidth,
	...
	\end{axis}
	\begin{scope}[mark size={0.3em}]
	\node[below right=0 of m-1-1.north west] {\ref{pgfplots:50}};
	\node[below right=0 of m-1-2.north west] {\ref{pgfplots:100}};
	\node[below right=0 of m-2-1.north west] {\ref{pgfplots:250}};
	\node[below right=0 of m-2-2.north west] {\ref{pgfplots:450}};
	\node[below right=0 of x800.north west] {\ref{pgfplots:800}};
	\end{scope}
	\end{tikzpicture}
	Shape of a ruck depends on relative excess length $\epsilon$: $\lambda \sim \epsilon^{1/7} \lambda^*$ and $A \sim \epsilon^{4/7} \lambda^*$.
	
	At contact: $A=H\equiv e-h\approx e/2$
	
	\[ \lambda \sim \epsilon^{-3/7} H \]
	%\[ \epsilon \sim \left(\frac{\lambda}{e}\frac{e}{H}\right)^{-7/3} \simeq \sqrt{1.6\%} \]
...
\begin{textblock}{3}(4,21.5)
\Subhead{Phenomenology}
\begin{itemize}
\item Global destabilisation
\item Labyrinth pattern
\item First wrinkle after $\approx\SI{13}{\minute}$ of swelling
\item Next generations = blisters
\end{itemize}
\end{textblock}

\begin{textblock}{3}(8,21.5)
\begin{itemize}
\item The same $\Delta P$ can be relaxed by\linebreak a Poiseuille flow
\[\Delta P \sim 12\eta\frac{\lambda}{H^2} u \sim 12\eta\frac{\lambda^2}{H^3} v\]
\item For the same wavelength
\[\frac{v_\text{Darcy}}{v_\text{Poiseuille}} \sim \frac{\ell_p^2\lambda^2}{hH^3} \approx 30\]
\end{itemize}
Flow through the pores dominate

\end{textblock}

\begin{textblock}{3}(12,19)
\Head{Viscosity}
\begin{tikzpicture}
	\begin{loglogaxis}[%
		scale only axis,
