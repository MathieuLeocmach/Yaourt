...
Here $\sigma$ is a Darcy pressure due to solvent flow through the gel (pore size $\ell_p \approx \SI{4}{\micro\metre}$)
...
\begin{axis}[
	...
	\draw[thick, ->] (ph2) -- (axis cs:90,0.5) node[midway, left] {Darcy} node[midway, right] {$v$};
	\draw[thick, ->] (pb2) -- (pb1) node[midway, above] {Poiseuille} node[midway, below] {$u \sim \frac{\lambda}{H} v$};
\end{axis}
